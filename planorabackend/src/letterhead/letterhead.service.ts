...
export class LetterheadService{
 ...
 async pdf(id:string,userId:string){const [settings,letter]=await Promise.all([this.settings(),this.getForUser(id,userId)]);if(letter.approval_status!=='APPROVED')throw new BadRequestException('Finalize or obtain Admin approval before downloading this official letter');return this.buildPdf(settings,letter)}
