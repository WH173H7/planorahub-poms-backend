...
export class AuditService {
  ...
  async log(params: AuditInput) {
    try {
      ...
      // Audit persistence should never turn an otherwise valid user action into a 500.
      // The migration still needs to be applied; this only keeps auth/workflows available
      // while surfacing the logging problem in backend logs.
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`Audit write failed for ${params.action}: ${message}`);
      return false;
    }
  }
  ...
  async logOnce(params: AuditInput, withinSeconds = 300) {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`Audit dedupe failed for ${params.action}: ${message}`);
      return false;
    }
  }
